...
// Was 12 hours. A client-side setInterval keep-alive (supabase-auth.js and
// each admin page) was added to refresh this before it lapsed, but that
// depends on a JS timer actually continuing to run for the whole gap —
// browsers throttle, freeze, or fully discard background tabs to save
// memory, and if the machine itself sleeps (lid closed) while a check runs,
// NO JavaScript executes at all until it wakes, so the interval simply
// cannot fire during that window. 12h was tight enough for an overnight or
// "come back later" wait to land right on that gap. 30 days removes the
// dependency on any timer surviving at all — the cookie only grants seeing
// the page SHELL (see this file's own note below); real data access is
// still gated per-request by the password header regardless of this value,
// so a long-lived cookie here is a visibility convenience, not a security
// loosening.
